...
const ChatBot = () => {
  ...
  const renderMarkdown = (text: string) => {
    // Escape HTML entities first so any user-supplied tags cannot execute,
    // then apply markdown transforms, then sanitize as a defense-in-depth layer.
    const escaped = text
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;")
  ...
  return (
    <div className="flex flex-col h-[600px] max-h-[80vh] bg-card rounded-2xl border border-border shadow-xl overflow-hidden">
      <div className="bg-primary px-6 py-4 flex items-center gap-3">
        ...
        <div>
          <h3 className="font-semibold text-primary-foreground text-sm">Assistant El May Djerba Immo</h3>
